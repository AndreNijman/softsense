...
| 7b | Pressure-CRUSH on TPU finger (trapped-air, worst case) | **PASS to 30 m**, degraded ≥100 m | If Fin Ray cells fail to flood (1 atm air inside, water outside), the differential ΔP loads the contact wall + ribs directly. Worst-case 2D plane-strain FEA: 0.41 MPa vM @ 30 m (62× yield margin, 0.6 mm wall sag, geometry intact) → 1.35 MPa @ 100 m (18.5× margin, 2.1 mm sag, geometry degraded). At ≥300 m the linear-elastic FEA is past validity (predicted sag > cell depth → self-contact dominates), but in any case the failure mode is **geometric collapse before material yield**, not catastrophic rupture. **Mitigation: pre-dive soak + actuation cycle** (load-critical, not optional, for dives >30 m). See `fea/UNDERWATER_FEA.md §3`. |
