...
| 7b | Pressure-CRUSH on TPU finger (trapped-air, worst case, 3D) | **GEOMETRY CRUSHED at any depth ≥10 m if cells don't flood** | The Fin Ray finger blade is a closed-cell foam (E_foam ≈ 0.5 MPa vs E_TPU = 40 MPa). If cells fail to flood, external water collapses the cells globally. 3D FEA: 1.6 mm contact-face sag @ 10 m, 4.8 mm @ 30 m (half the finger thickness), >10 mm @ 100 m. Material doesn't yield until ~200 m, but the finger geometry is wrecked well before that — wrap fails. **Mitigation: pre-dive cell flooding is LOAD-CRITICAL from depth zero**, not just deep dives. (An earlier 2D plane-strain analysis suggested 30 m was OK; that analysis was wrong — εz=0 hid the dominant foam-collapse mode.) See `fea/UNDERWATER_FEA.md §3`. |
...
| `finger_R` / `finger_L` (Fin Ray) | **Ether-based TPU ~95A** — selected: **eSUN eTPU-95A** (also NinjaFlex, BASF ether grades) | **Reject ester-based TPU** — it hydrolyzes in sustained/warm immersion and crumbles. Ether-TPU is hydrolysis-stable. TPU absorbs a little water and softens slightly (re-check grip if tuned tight). **eSUN eTPU-95A:** eSUN markets *"good hydrolysis resistance"* + weather/outdoor use (the polyether trait) → reads as polyether, suitable here. eSUN does **not** print the polyol chemistry on its TDS, so for mission-critical immersion, confirm polyether with eSUN or soak-test first. Profile + FEA: `PRINT_PROFILE_P1S_TPU.md`. |
...
- **Flood the Fin Ray finger cells — MANDATORY at ALL depths** (3D FEA in
  `fea/UNDERWATER_FEA.md §3`): the blade is a closed-cell foam (E_foam ≈
  0.5 MPa). If cells trap air, even shallow water collapses them: contact
  face sags 1.6 mm @ 10 m, 4.8 mm @ 30 m (half the finger thickness),
  geometry wrecked. Procedure: submerge slowly so air vents through the
  +Z / −Z cell openings, soak ~30 s, then cycle open↔close ONCE underwater
  to flush residual air. Pinch the blade gently between fingers if a cell
  visibly retains a bubble. Do **not** assume "shallow = safe" — the
  failure mode is geometric collapse, not material yield, so it happens
  long before any pressure that would worry the TPU itself.
