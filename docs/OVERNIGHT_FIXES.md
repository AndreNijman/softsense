...
## Renders regenerated to match the corrected analyses

- **`motor/pictures/torque_chain.png` / `slip_margin.png`** — re-rendered
  with the three crown T_safe bounds (3D = headline, radial 2D, single-
  station 2D) plus the proposed re-size. Shows the operating force band
  on each tier.
- **`grip/pictures/*.png`** (7 figures: family_scores, per_condition,
  baseline_gate, sensitivity_winner, sensitivity_heatmap, crosshatch_sweep,
  tier2_fea) — regenerated via `grip/scripts/make_figures.py` against
  the current model state.
- **`renders/gripper_hero_open.png` / `_closed.png` / `gripper_motion.gif`
  / `gripper_xray.png` / `gripper_xray.gif`** — regenerated by
  `scripts/render_full_system.sh` (CAD unchanged from gripper.py, but
  regenerated for completeness).
- **`renders/adapters/*.png`** — already regenerated earlier in this branch
  (the unibody-fit-up commit at the start).
- **`fea/iterations/_compare_prod_vs_finray2/compare.png` / `.gif`** —
  NEW side-by-side wrap-stage render of the two stable Fin Ray candidates
  used in the B3 cross-design rank check. Shows finray2 reaching higher
  grip at the same closure with lower peak vM per N.
- **`fea/iterations/_locking_nu{40,42,45,48}/wrap_stages.png`,
  `_mesh_nl{3,5,8}/wrap_stages.png`, `_oprank_{finray2,flexure}_lowF/wrap_stages.png`**
  — auto-generated by `iter_harness.py` for each sweep point; show the
