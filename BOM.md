...
| **Pressure canister for the actuator** (T1–T3) | 1 | Houses the chosen actuator (XW540 / XM540 / STS3250 / STS3215) for any non-trivially-wet deployment — **T2 (≤ 30 m) needs a canister even for the IP68 XW540** (IP68 = 1 m freshwater only). **Recommended kit (Blue Robotics 3" locking series, the de-facto small-ROV standard):** (a) tube — [3" cast-acrylic, 240 mm length, 150 m depth rating, **USD 25**](https://bluerobotics.com/store/watertight-enclosures/locking-series/wte-locking-tube-r1-vp/) (Part `BR-102649-240`; fits the 117 mm XW540/XM540 body + connector/cable bend; ≥ 5× depth margin over T2); (b) end caps — two **aluminium 6061-T6 anodised, 1000 m rated, USD 34 each**: ([**`BR-100949-999`** — "blank" SKU = *no factory holes as shipped*, **drilled in build to Ø14 H7 in the centre for the shaft-seal bore** per `motor/ROV_INTEGRATION.md` §2d Option A](https://bluerobotics.com/store/watertight-enclosures/locking-series/wte-end-cap-vp/)) for the **wet-side / shaft-exit end** + ([**`BR-100949-004`** — 4 × M10 factory penetrator holes](https://bluerobotics.com/store/watertight-enclosures/locking-series/wte-end-cap-vp/)) for the **dry-side / cable end** (2 of the M10s carry WetLink penetrators below; the other 2 take Blue Robotics M10 sealing plugs, ~USD 3 each); (c) **cable penetrators** — [Blue Robotics WetLink](https://bluerobotics.com/store/cables-connectors/penetrators/wetlink-penetrator/), ~USD 13 each, qty ≥ 2 (one bus + power, one spare/vent). **Assembled cost ≈ USD 120 (≈ AUD 185)** for the acrylic build; **aluminium tube upgrade ~ USD 100+** for 1000 m + long-immersion durability (acrylic is rated for *short-term* submersions < ~2 weeks per Blue Robotics). **Shaft exit from the canister:** an additional SKF CR 8×14×4 NBR radial lip seal (~USD 5) + a Ø8 mm polished anodised-aluminium adapter shaft drives the gripper D-coupler through one of the end-cap M10 holes drilled to Ø14 H7 — see `motor/ROV_INTEGRATION.md` §2d for both architectures (Option A lip seal for T1/T2, Option B magnetic coupling for T3), part numbers, parasitic-torque table, and failure modes. For T3, the magnetic-coupling fallback's dry pod uses the same enclosure family (`motor/SELECTION.md` D8). |
