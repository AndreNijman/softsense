...
| `docs/PRINT_PROFILE_P1S_TPU.md` + `profiles/*.json` | **Importable Bambu Studio profiles** (filament + process) for printing the fingers in **Bambu TPU 95A HF on a P1S / 0.4 mm hardened nozzle** — every setting + rationale, and the FEA re-check on Bambu's *measured* ISO 527 data (9.8 MPa in-plane modulus, 27.3 MPa strength) confirming the force-targeted margins hold. |
